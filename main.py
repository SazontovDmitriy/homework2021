...
def meetMonster1():
    ...
    monsterLvl1 = r.randint(1, 3)
    monsterHp1 = monsterLvl1
    monsterDmg1 = monsterLvl1 * 2 - 1
    monsters = ["Warlock", "Demonic Clown", "Horserider", "Marmol", "Litlegobl"]

    monster = r.choice(monsters)

    print(
        "Ты набрел на монстра - {0}, у него {1} уровень, {2} жизней и {3} урона.".format(monster, monsterLvl1,
                                                                                             monsterHp1,monsterDmg1))
    while monsterHp1 > 0:
        choice = input("Что будешь делать (атака/бег): ").lower()

        if choice == "атака":
            monsterHp1 -= damage
            print("Ты атаковал монстра и у него осталось", monsterHp1, "жизней.")
        elif choice == "бег":
            chance = r.randint(3, monsterLvl1)
            if chance == 4:
                print("Тебе удалось сбежать с поля боя!")
                break
            else:
        ...
        loot = r.randint(0, 5) + monsterLvl1
        coins += loot
        print("Тебе удалось одолеть монстра, за что ты получил", loot, "монет.")
        printCoins()
    ...
    while monsterHp1 > 0:
        ...
        if choice == "атака":
            ...
            chance = r.randint(2, monsterLvl1)
            if chance == 3:
                print("Тебе удалось сбежать с поля боя!")
                break
            else:
        ...
        loot = r.randint(5, 8) + monsterLvl1
        coins += loot
        print("Тебе удалось одолеть монстра, за что ты получил", loot, "монет.")
        printCoins()
